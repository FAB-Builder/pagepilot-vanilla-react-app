...
function Countdown({ to }: { to: string }) {
  ...
  if (!parts)
    return (
      <span className="text-xs font-semibold text-green-600">Starting now</span>
    );
    ...
    <div className="flex items-center gap-1.5">
      {parts.d > 0 && (
        <div className="flex min-w-[44px] flex-col items-center rounded-lg border border-indigo-100 bg-white px-2.5 py-1.5 shadow-sm">
          <span className="text-base font-bold leading-none text-slate-800">
      ...
      {(["h", "m", "s"] as const).map((key) => (
        <div
          key={key}
          className="flex min-w-[44px] flex-col items-center rounded-lg border border-indigo-100 bg-white px-2.5 py-1.5 shadow-sm"
        >
          <span className="text-base font-bold leading-none text-slate-800">
      ...
      <div className="p-4">
        {loading ? (
          ...
          <div className="rounded-lg border border-dashed border-indigo-200 bg-indigo-50/50 px-4 py-5 text-center">
            ...
            <p className="text-sm font-medium text-slate-500">
              No upcoming webinars in the next 36 hours
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {items.map(({ event, nextOccurrence }) => (
              <div
                key={event._id}
                className="rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white px-4 py-4"
              >
    ...
    <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-500">
          Live demo — Page Pilot demo workspace
        </span>
        <button
          type="button"
          onClick={load}
  ...
  return (
    <Section id="live-upcoming" title="Live & Upcoming API">
      ...
      <DemoBlock
        ...
        description={
          <>
            Pass <Code>inNextHours</Code> to control the look-ahead window.
          </>
        }
      ...
      <h3 className="pt-4 text-base font-bold text-slate-800">
        Currently running
      </h3>
      <p>
